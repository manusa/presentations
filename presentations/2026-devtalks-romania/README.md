...
2026-devtalks-romania/
  index.html                 single-page deck (21 slides) — markup + inlined JS components only
  README.md                  this file
  styles/                    split out of index.html; loaded in order via <link> tags in <head>
    ...
    s-title.css              .s-title + corner ticks (used by the trailing exploration slide)
    s-about.css              legacy single-portrait .s-about (no slide uses it; kept for parity with upstream bundle)
    ...
    s-content.css            .s-content (template demo)
    s-compare.css            .s-compare two-column (template demo)
    s-quote.css              .s-quote big quote (template demo)
    s-stat.css               .s-stat big-number (template demo)
    s-code.css               .s-code terminal (template demo)
    s-diagram.css            .s-diagram placeholder (template demo)
    s-fullbleed.css          .s-fullbleed (template demo)
    s-closing.css            .s-closing + @media print refinements
...
- 21 slides total: the real talk arc (slides 1–12, Title → Acts I/II/III → Legacy framing) followed by 8 template-demo slides (Content / Compare / Quote / Stat / Code / Diagram / Full-bleed / Closing) and 1 trailing exploration ("15 Title — Original"). The template-demo and exploration sections at the tail are still here from the upstream handoff; trim them once the speaking order is locked.
...
- **Trim trailing template-demo + exploration slides** once the talk's slide order is final — they're not part of the actual talk. The matching `styles/s-{content,compare,quote,stat,code,diagram,fullbleed,closing}.css` and `styles/s-about.css` files (and their `<link>` lines in `index.html`) can be removed at the same time — they exist only for those template-demo / legacy sections.
