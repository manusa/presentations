...
2026-valenciajug/
  ...
  assets/
    valenciajug-logo.svg     title-slide brand mark (bat traced from the official logo; wordmark "VALENCIAJUG" + "JAVA USER GROUP" subtitle reset as Arial Bold/Regular glyph outlines, not bitmap-traced; dark-mode recolor, transparent bg)
...
- **Brand assets.** `valenciajug-logo.svg` (bat traced from the official logo; the "VALENCIAJUG" wordmark and "JAVA USER GROUP" subtitle are set as real-font glyph outlines — Arial Bold / Arial Regular, fitted to the original's cap-height, baseline and per-word tracking — rather than bitmap-traced, so the lettering is crisp instead of wobbly; dark-mode recolor, transparent bg), `favicon.svg`, `social-card.png` (from the flyer), `assets/logos/xplore-group.svg`, and a regenerated `qr-links.svg` pointing at the Valencia links page. `devtalks-logo.png` dropped.
